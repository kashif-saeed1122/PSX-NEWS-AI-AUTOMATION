...
# Dual-write to Postgres alongside the JSON-based path.
try:
    from db.repository import save_pipeline_run, save_news_briefing, safe
    _DB_ENABLED = True
except Exception:
    _DB_ENABLED = False
...
def run_news_analysis(days_back: int = 1, pipeline_run_id=None) -> dict:
    ...
    # Dual-write to Postgres. If no pipeline_run_id was supplied by the caller,
    # create one here so the briefing has a parent.
    if _DB_ENABLED:
        run_id = pipeline_run_id
        if run_id is None:
            run_id = safe(save_pipeline_run, "legacy_news_only", response.usage.total_tokens, "ok")
        if run_id is not None:
            briefing_id = safe(save_news_briefing, run_id, briefing)
            if briefing_id is not None:
                briefing["_meta"]["briefing_id"] = str(briefing_id)
                briefing["_meta"]["pipeline_run_id"] = str(run_id)
