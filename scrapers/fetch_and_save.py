...
from scrapers.nccpl_scraper import fetch_insider_transactions, fetch_fipi_lipi
from scrapers import price_history as ph
from db.repository import (
    upsert_stocks_from_psx,
    upsert_stock_prices,
    upsert_index_snapshots,
...
OUTPUT_DIR = os.path.join(_PROJECT_ROOT, "backend", "data")
...
def fetch_google_news():
    ...
    data = save_json("01_google_news.json", result)
    safe(upsert_news_articles, data.get("articles", []), "Google News")
    return data
...
def fetch_dawn():
    ...
    data = save_json("02_dawn_business.json", result)
    safe(upsert_news_articles, (data.get("rss") or {}).get("articles", []), "Dawn Business")
    return data
...
def fetch_profit_pk():
    ...
    data = save_json("03_profit_pakistan.json", result)
    safe(upsert_news_articles, (data.get("rss") or {}).get("articles", []), "Profit.pk")
    return data
...
def fetch_general_news():
    ...
    data = save_json("05_general_news.json", result)
    safe(upsert_news_articles, data.get("articles", []), "General News")
    return data
...
def fetch_psx_portal():
    ...
    data = save_json("04_psx_data_portal.json", result)
    stocks = data.get("all_stocks") or []
    safe(upsert_stocks_from_psx, stocks)
    safe(upsert_stock_prices, stocks)
    safe(upsert_index_snapshots, data.get("indices_live") or {})
    return data
...
def fetch_nccpl_insider():
    ...
    save_json("06_nccpl_insider.json", data)
    # FIPI is paired with this in nccpl_signals — also persist what we have so far.
    safe(save_nccpl_signals, data, {})
    return data
...
def fetch_nccpl_fipi():
    ...
    save_json("07_nccpl_fipi.json", data)
    safe(save_nccpl_signals, {}, data)
    return data
