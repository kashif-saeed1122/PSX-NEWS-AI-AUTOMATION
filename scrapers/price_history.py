...
# Dual-write to Postgres alongside the JSON store. Wrapped in `safe` at the
# call site so a DB outage never breaks the file-based path.
try:
    from db.repository import upsert_stocks_from_psx, upsert_stock_prices, safe
    _DB_ENABLED = True
except Exception:
    _DB_ENABLED = False
...
def append_today(psx_data: dict) -> dict:
    ...
    # Dual-write to Postgres (idempotent upsert per symbol-day).
    if _DB_ENABLED:
        all_stocks = psx_data.get("all_stocks") or []
        safe(upsert_stocks_from_psx, all_stocks)
        n_db = safe(upsert_stock_prices, all_stocks)
        if n_db:
            print(f"  [price_history] {today}: also wrote {n_db} rows to Postgres stock_prices")
